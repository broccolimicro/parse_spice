...
token number::consume(tokenizer &tokens, void *data)
{
	...
	if (type == decimal && (character == 'e' || character == 'E') && (tokens.peek_char(2) == '-' || tokens.peek_char(2) == '+' || (tokens.peek_char(2) >= '0' && tokens.peek_char(2) <= '9')))
	{
		tokens.next_char();
		character = tokens.peek_char(1);
	...
	if (string("aAfFpPnNuUmMkKxXgG").find(character) != string::npos) {
		tokens.next_char();
		character = tokens.peek_char(1);
	}
